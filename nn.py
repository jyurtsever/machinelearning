...
class MatrixVectorAdd(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        ones = np.ones(inputs[0].shape[0])
        return [gradient, np.dot(gradient.T, ones)]
...
class SquareLoss(FunctionNode):
    ...
    def forward(inputs):
        ...
        return np.mean(.5*np.square(diff))
    @staticmethod
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        diff = inputs[0] - inputs[1]
        return [diff*gradient*1/diff.size, -diff*gradient*1/diff.size]
